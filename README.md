...
* The front-end application is an SPA in React and was built using Next.js. The Next.js website says there is a minimum [Node.js](https://nodejs.org/en) version of 20.9. My IDE was Visual Studio Code. Start the front-end project from the root directory with `npm run dev`. Navigate to `http://localhost:3000/ticket-management` for the relevant application.
...
* Task-based - The system is task based, with the exception of the requested "update ticket" functionality. That would ordinarily be split into tasks that communicate *intent*, such as "Alter description". The best example of this is the assign user functionality.
  * Note: There may be some confusing functionality around the updating of tickets and their statuses. Neither setting a ticket to open nor in progress work, while closing a ticket works. This is somewhat intentional and I would be happy to discuss this. But it is clear in the `AssignUserHandler` that this functionality is intentionally missing. If this were a real project, I would have a discussion with the domain experts/end users about how to implement this functionality more effectively.
